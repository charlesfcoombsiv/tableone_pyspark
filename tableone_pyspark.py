...
def tableone_pyspark(df, spark_session, col_to_strat="", cols_to_analyze_list=[], beautify=False, p_values=False):
    ...
    if col_to_strat == "":
        ...
        # "add" function here was imported from operations so SQL "add" function doesn't override it
    ...
    for col_i in cols_to_analyze_list:
        ...
        # Prepare data to be analyzed in a dataframe.
        # Add "MISSING" to col_i LATER depending on it continuous or categorical analysis.
        # Change col_to_strat values to be valid as column's name. Replace null with "MISSING".
        if col_to_strat == '':
            initial_df = df.select(col_i)
        else:
            initial_df = df.select(col_i, col_to_strat).fillna("MISSING", subset=[col_to_strat]) \
                .withColumn(col_to_strat, regexp_replace(col_to_strat, ' ', '_')) \
                .withColumn(col_to_strat, regexp_replace(col_to_strat, r'[^\x00-\x7F]+', '?'))
        ...
        if col_type == 'string':
            ...
            # Fill in null values with "MISSING" and broadcast.
            initial_df = broadcast(initial_df.fillna("MISSING", subset=[col_i]))
        ...
        # conduct continuous analysis
            ...
            # Broadcast dataframe to be analyzed.
            # Done this way because categorical needs nulls for col_i to be "MISSING"
            # but continuous needs nulls to remain null.
            initial_df = broadcast(initial_df)
...
def analysis_continuous(col_i, initial_df, col_to_strat, idx):
    ...
    if col_to_strat != "":
        # per pivoted column values
        df_std = df_std.join(initial_pivoted.agg({col_i: 'stddev'}).withColumn("Values", lit('stddev')), "Values",
                             "inner")
    ...
    if col_to_strat != "":
        # per pivoted column values
        df_25quar = df_25quar.join(
            initial_pivoted.agg(F.expr('percentile_approx({}, 0.25,  {})'.format(col_i, ct_plus_one))).withColumn(
                "Values", lit('25th percentile')), "Values", "inner")
    ...
    if col_to_strat != "":
        # per pivoted column values
        df_50quar = df_50quar.join(
            initial_pivoted.agg(expr('percentile_approx({}, 0.50,  {})'.format(col_i, ct_plus_one))).withColumn(
                "Values", lit('50th percentile')), "Values", "inner")
    ...
    if col_to_strat != "":
        # per pivoted column values
        df_75quar = df_75quar.join(
            initial_pivoted.agg(expr('percentile_approx({}, 0.75,  {})'.format(col_i, ct_plus_one))).withColumn(
                "Values", lit('75th percentile')), "Values", "inner")
